...
class APConfig
{
    ...
    static boolean checkParameters(params, log)
    {
        ...
        if (notExists(fastaReferencePath(params)))
        {
            log.error "FASTQ reference file '${fastaReferencePath(params)}' does not exist."
            errors = true
        }
        if (params.createCoverage && notExists(genomeSizesPath(params)))
        {
            log.error "Genome sizes file '${genomeSizesPath(params)}' does not exist."
            errors = true
        }
        if (params.rnaseqMetrics && notExists(referenceRefFlatPath(params)))
        {
            log.error "Reference annotation refflat file '${referenceRefFlatPath(params)}' does not exist."
            errors = true
        }
        switch (params.aligner)
        {
            case 'bwa':
                if (notExists("${bwaIndexPath(params)}.pac"))
                {
                    log.error "BWA index files '${bwaIndexPath(params)}' do not exist."
                    errors = true
                }
                break
            ...
            case 'bwamem2':
            case 'bwa_mem2':
                if (notExists("${bwamem2IndexPath(params)}.pac"))
                {
                    log.error "BWAmem index files '${bwamem2IndexPath(params)}' do not exist."
                    errors = true
                }
                break
            ...
            case 'bowtie':
            case 'bowtie2':
                if (notExists("${bowtie2IndexPath(params)}.1.bt2") &&
                    notExists("${bowtie2IndexPath(params)}.1.bt2l"))
                {
                    log.error "Bowtie index files '${bowtie2IndexPath(params)}*' do not exist."
                    errors = true
                }
                break

            case 'star':
                if (notDirectory(starIndexPath(params)))
                {
                    log.error "STAR genome directory '${starIndexPath(params)}' does not exist."
                    errors = true
                }
                break
        }
    ...
    static void displayParameters(params, log)
    {
        ...
        log.info "FASTA file: ${fastaReferencePath(params)}"
        if (params.createCoverage)
        {
            log.info "Genome sizes: ${genomeSizesPath(params)}"
        }
        if (params.rnaseqMetrics)
        {
            log.info "Reference annotations (refflat): ${referenceRefFlatPath(params)}"
            log.info "Strand specificity: ${rnaseqStrandSpecificity(params)}"
        }
        switch (params.aligner.toLowerCase())
        {
            case 'bwa':
                log.info "BWA index: ${bwaIndexPath(params)}*"
                break
            ...
            case 'bwamem2':
            case 'bwa_mem2':
                log.info "BWAmem2 index: ${bwamem2IndexPath(params)}*"
                break

            case 'bowtie':
            case 'bowtie2':
                log.info "Bowtie2 index: ${bowtie2IndexPath(params)}*"
                break

            case 'star':
                log.info "STAR index: ${starIndexPath(params)}"
                break
        }
    }
